...
public abstract class ProtobufSpecificationParser {
  ...
  static SpecificationProtos.Specification parseStringToProtoSpec(String specString)
    ...
    SpecificationProtos.Specification.Builder specBuilder =
        SpecificationProtos.Specification.newBuilder();
    TextFormat.getParser().merge(specString, specBuilder);
    return specBuilder.build();
  }

  static Specification buildModel(SpecificationProtos.Specification specification)
      throws Exception {
    ImmutableMap<String, ImmutableSet<String>> stateVarMap =
        parseStateVector(specification.getStateVarBlock());
    ImmutableSet<StateSpec> stateSpecs =
        stateVarMap
            .entrySet()
            .stream()
            .map(e -> StateSpec.create(e.getKey(), e.getValue()))
    ...
    ImmutableMap<String, Predicate> aliases =
        parseAliases(specification.getAliases(), stateVarMap, actionNames);

    ImmutableMap<String, Set<String>> mutableControlledResources =
        parseControlledResources(specification.getResourceBlock());

    ImmutableSet<ActionSpec> actionSpecs =
        parseActions(
            specification.getActionsBlock(),
            stateVarMap,
            actionNames,
            aliases,
            mutableControlledResources);
    ...
    ImmutableSet<MutuallyExclusiveActions> mutuallyExclusiveActions =
        mutableControlledResources
            ...
            .map(MutuallyExclusiveActions::create)
            .collect(ImmutableSet.toImmutableSet());
  ...
  private static ImmutableMap<String, ImmutableSet<String>> parseStateVector(
    ...
    for (SpecificationProtos.StateVariable variable : stateVarBlock.getStateVariablesList()) {
      if (knownStateVariables.contains(variable.getName())) {
        throw new Exception(
            "The state variable " + variable.getName() + " was declared more than once.");
      }
      if (variable.getValuesCount() < 1) {
        throw new Exception(
            "The state variable " + variable.getName() + " needs at least one possible value.");
      }
  ...
  private static ImmutableSet<String> parseActionNames(SpecificationProtos.ActionBlock actionBlock)
      throws Exception {
  ...
  private static ImmutableMap<String, Predicate> parseAliases(
      SpecificationProtos.AliasBlock aliasBlock,
      ImmutableMap<String, ImmutableSet<String>> stateVarMap,
    ...
    for (SpecificationProtos.Alias alias : aliasBlock.getAliasesList()) {
      ...
      Predicate predicate =
          PredicateParser.parsePredicate(
              alias.getValue(), stateVarMap, actionNames, resultBuilder.build());
  ...
  private static ImmutableSet<ActionSpec> parseActions(
      SpecificationProtos.ActionBlock actionBlock,
      ImmutableMap<String, ImmutableSet<String>> stateVarMap,
      ImmutableSet<String> actionNames,
      ImmutableMap<String, Predicate> aliases,
      ImmutableMap<String, Set<String>> controlledResources)
      throws Exception {
    ...
    for (SpecificationProtos.Action action : actionBlock.getActionsList()) {
      for (String resource : action.getResourcesList()) {
        ...
        controlledResources
            .get(resource)
            .add(action.getName()); // Add the action to the controller resource it belongs to
      }
      ...
      resultBuilder.add(
          ActionSpec.create(action.getName(), action.getCost(), changeables, precondition, effect));
    }
  ...
  private static ImmutableSet<Assumption> parseAssumptions(
      SpecificationProtos.AssumptionsBlock assumptionsBlock,
      ImmutableMap<String, ImmutableSet<String>> stateVarMap,
      ImmutableSet<String> actionNames,
      ImmutableMap<String, Predicate> aliases)
      throws Exception {
  ...
  private static Goals parseGoals(
      SpecificationProtos.GoalBlock goalBlock,
      ImmutableMap<String, ImmutableSet<String>> stateVarMap,
      ImmutableSet<String> actionNames,
      ImmutableMap<String, Predicate> aliases)
      throws Exception {
